...
const deleteCart = async (req, res) => {
    try {
        ...
        const updatedCart = await cartModel.findOneAndUpdate(
            { user: userId }, 
            { $pull: { cartData: { eventName: eventName } } }, 
            { new: true } 
        );

        if (!updatedCart) {
            return res.status(401).send({ msg: "Cart not found" });
        }

        return res.status(200).send({ msg: "Event removed from cart successfully", cart: updatedCart });

    } catch (error) {
        return res.status(500).send({ msg: "Something went wrong. Please try again later.", error });
    }
};
