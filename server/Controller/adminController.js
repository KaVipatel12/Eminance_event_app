...
const participantList = async (req, res) => {
    try {
        ...
        let participantWithPurchasedEvents = users.filter((data) => data.purchaseProduct?.length > 0).map(user => ({
                    email: user.email,
                    userId: user._id,
                    phone: user.phone,
        ...
        if(participantWithPurchasedEvents.length > 0){
           return res.status(201).send({msg : participantWithPurchasedEvents}); 
        }
        res.status(401).send({msg : "No participant has purchased any course yet"}); 
}catch(err){
